locals {
  ...
  rolename = local.create_role ? coalesce(var.rolename, var.function_name, "*") : null
...
resource "aws_iam_policy" "additional_jsons" {
  ...
  name   = "${local.rolename}-${count.index}"
  path   = var.policy_path
